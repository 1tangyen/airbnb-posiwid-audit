...
02_data_cleaning.md         — Cleaning rules, row counts, V2 stratification variables
03_analysis_signals.md      — Methods per signal, V2 stratified analysis design
04_results.md               — V1 cross-city results + V2 stratified results
05_dashboard.md             — Dashboard build notes + V2 stratified chart plans
08_outlier_detection.md     — Outlier detection & description impact methodology
scripts/
  01_download.sh            — Automated data download
  02_clean.py               — Data cleaning pipeline
  03_analyze.py             — Signal analysis (V1 + V2 stratification)
  04_generate_dashboard.py  — Dashboard HTML generation
  05_outlier_reviews.py     — Review sentiment outlier detection
  06_outlier_price.py       — Price outlier detection by neighbourhood
  07_description_impact.py  — Description features → price/review impact
data/                       — Raw data (gitignored, ~226MB)
output/
  dashboard.html            — Interactive results dashboard
  analysis_results.json     — Full analysis output (JSON)
  outlier_reviews.json      — Review outlier detection results
  outlier_price.json        — Price outlier detection results
  description_impact.json   — Description impact analysis results
...
## V2 — Host Stratification (In Progress)

### Why V2?

V1 treated all hosts as a homogeneous group. This leaves the strongest alternative explanation untested: **are the patterns we found driven by platform design, or by host type?** If score inflation and the Hidden Transcript concentrate in commercial operators (entire-home, multi-listing hosts), the conclusion shifts from "the platform produces compliance" to "the platform enables certain hosts to game the system."

### Stratification Dimensions

| Dimension | Variable | Groups |
|-----------|----------|--------|
| Operating model | `room_type` | Entire home / Private room / Shared room |
| Host scale | `calculated_host_listings_count` | Single (1) / Small multi (2-4) / Medium multi (5-9) / Large multi (10+) |
| Competition intensity | Neighbourhood listing density | High / Medium / Low tercile |
| Property category | `property_type` | Apartment / House / Condo / Room / Other |
| Intra-host consistency | `host_id` grouping | Same-host vs cross-host description similarity |

### Current Status

| Component | Status |
|-----------|--------|
| Methodology (02, 03 docs) | ✅ Updated |
| Analysis script (`03_analyze.py`) | ✅ V2 stratification added and executed |
| V2 results in 04_results.md | ✅ All tables populated with data |
| Modified POSIWID verdict | ✅ Written |
| Dashboard V2 charts | ⬜ Planned, not yet generated |

### Key V2 Findings

| Signal | V2 Result | Surprise Level |
|--------|----------|---------------|
| **B — Score Inflation** | Individual hosts inflate MORE (93-98%) than professional operators (74-87%) | ⭐ Inverts the expected pattern |
| **C — Hidden Transcript** | Large operators: 7-11% negative rate vs 4-5% for solo hosts. Boston large: 10.56% | ⭐ Strongest new finding |
| **D — Homogenization** | Same-host similarity 7-9x higher than cross-host (0.35-0.46 vs 0.05) | Partially revises Winner interpretation |

---

## POSIWID Verdict (V2)

> *The purpose of Airbnb's review system is what it does*: it produces universal score inflation that captures individual hosts most completely (93-98% above 4.5), while enabling commercial operators to maintain viable scores (74-87%) despite delivering measurably worse experiences (7-11% negative text). The system does not merely reduce booking friction — it creates a **subsidy for scale**, where the rating system's inability to differentiate benefits those who need differentiation least.

See `04_results.md § V2 — Alternative Explanation Test` for full stratified analysis.
